...
import { deletePrescription } from "../../prescricoes/actions";

export function PrescriptionMenu({ prescriptionId, patientId }: { prescriptionId: string; patientId: string }) {
      ...
      <AnimatePresence>
        {open && (
          <>
            ...
            >
              <Link
                href={`/medico/prescricoes/${prescriptionId}/editar`}
              ...
              <button
                ...
                disabled={pending}
                onClick={() => {
                  if (!confirm("Excluir essa prescrição? Essa ação não pode ser desfeita.")) return;
                  setOpen(false);
                  startTransition(() => {
                    void deletePrescription(prescriptionId, patientId);
                  });
                }}
