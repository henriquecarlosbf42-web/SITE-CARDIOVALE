...
import { cancelAppointment, deleteAppointment } from "@/app/(medico)/medico/agenda/actions";

export function AppointmentMenu({ appointmentId, patientId }: { appointmentId: string; patientId: string }) {
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 z-20 mt-2 w-44 overflow-hidden rounded-xl border border-ink-100 bg-surface p-1 shadow-soft"
            >
              <Link
                href={`/medico/agenda/${appointmentId}/editar`}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setOpen(false);
                  startTransition(() => {
                    void cancelAppointment(appointmentId, patientId);
                  });
                }}
                className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-ink-900 transition-colors hover:bg-surface-soft disabled:opacity-60"
              >
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setOpen(false);
                  startTransition(() => {
                    void deleteAppointment(appointmentId, patientId);
                  });
                }}
